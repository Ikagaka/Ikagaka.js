				...
				return function(num){
					...
					task.add({
						...
						,task:function(){
							...
							var that = this;
							if(this.curScope === 0){
								$("#" + PID + ">.scope" + that.curScope  + ">.blimp").css("right",that.img.width - 200 + "px").css("top", -130 + "px");
							}else{
								$("#" + PID + ">.scope" + that.curScope  + ">.blimp").css("left",that.img.width + "px").css("top", 50 + "px");
							}
						}
					});
					...
					if(typeof surface.collision !== "undefined"){
						...
						for(var i = 0; i < surface.collision.length; i += 1){
							...
							$("#" + PID + ">.scope" + curScope + ">.collision").append(
								...
								.bind("touchmove",function(){move[0].task();})
							);
						}
					}
			...
			scopeObj.blimp = (function(){
				...
				return function(num){
					...
					task.add({
						...
						,img:img
						,task:function(){
							base($("#" + PID + ">.scope0>.blimp>canvas")[0],transImg(this.img));
					...
					task.add({
						...
						,img:img2
						,task:function(){
							base($("#" + PID + ">.scope" + this.curScope + ">.blimp>canvas")[0],transImg(this.img));
					...
					img2.onload = function(){task.work();};
