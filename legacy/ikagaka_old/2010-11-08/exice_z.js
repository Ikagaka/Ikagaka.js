window.exice_z = {
	"homeurl":"../exice_z/"
	,"shell":{
		"master":{
			"descript":{
				"craftman":"kugelscheriber"
				,"id":"scribble"
				,"name":"master"
				,"type":"shell"
				,"craftmanw":"くーげるしゅれいばー"
				,"craftmanurl":"http://altenotiz.sakura.ne.jp/"
				,"sakura.balloon.offsetx":15
				,"sakura.balloon.offsety":45
				,"kero.balloon.offsetx":5
				,"kero.balloon.offsety":50
			}
			,"sakura.surface.alias":[
				[0,22,23]
			]
			,"surface":[
				{
					"src":"shell/master/surface0000.PNG"
					,"collision":[
						[80,62,143,95,"Head"]
						,[91,111,139,142,"Face"]
						,[89,181,140,216,"Bust"]
						,[184,187,211,238,"LeftST"]
					]
					,"interval":[
						{
							"type":"sometimes"
							,"pattern":[
								[-1,6,"overlay",0,0]
								,[61,5,"overlay",85,100]
								,[60,5,"overlay",85,100]
								,[-1,6,"overlay",0,0]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0001.PNG"
					,"collision":[
						[94,73,156,106,"Head"]
						,[102,110,154,141,"Face"]
						,[109,176,146,212,"Bust"]
						,[184,213,208,281,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0002.PNG"
					,"collision":[
						[73,68,122,102,"Head"]
						,[82,106,129,145,"Face"]
						,[99,161,149,202,"Bust"]
						,[179,161,213,239,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0003.PNG"
					,"collision":[
						[85,104,134,138,"Head"]
						,[89,144,138,174,"Face"]
						,[107,191,149,221,"Bust"]
						,[175,175,209,246,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0004.PNG"
					,"collision":[
						[86,97,156,128,"Head"]
						,[108,136,157,168,"Face"]
						,[117,187,154,221,"Bust"]
						,[202,230,245,303,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0005.PNG"
					,"collision":[
						[95,67,157,103,"Head"]
						,[104,116,158,143,"Face"]
						,[106,174,155,209,"Bust"]
						,[182,189,209,274,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0006.PNG"
					,"collision":[
						[95,67,157,103,"Head"]
						,[104,116,158,143,"Face"]
						,[106,174,155,209,"Bust"]
						,[182,189,209,274,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0007.PNG"
					,"collision":[
						[86,61,144,101,"Head"]
						,[90,107,146,145,"Face"]
						,[87,178,140,200,"Bust"]
						,[181,164,211,240,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0008.PNG"
					,"collision":[
						[80,62,143,95,"Head"]
						,[91,111,139,142,"Face"]
						,[89,181,140,216,"Bust"]
						,[184,187,211,238,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0009.PNG"
					,"collision":[
						[80,62,143,95,"Head"]
						,[91,111,139,142,"Face"]
						,[89,181,140,216,"Bust"]
						,[184,187,211,238,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0010.PNG"
				}
				,{
					"src":"shell/master/surface0011.PNG"
				}
				,{
					"src":"shell/master/surface0012.PNG"
				}
				,{
					"src":"shell/master/surface0013.PNG"
				}
				,{
					"src":"shell/master/surface0014.PNG"
				}
				,{
					"src":"shell/master/surface0015.PNG"
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[16,20,"overlay",0,0]
								,[16,14,"overlay",0,5]
								,[16,12,"overlay",0,10]
								,[16,10,"overlay",0,15]
								,[16,8,"overlay",0,20]
								,[16,5,"overlay",0,35]
								,[16,5,"overlay",0,40]
								,[16,5,"overlay",0,60]
								,[16,5,"overlay",0,80]
								,[16,5,"overlay",0,100]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0016.PNG"
				}
				,{"src":"shell/master/surface0017.PNG"}
				,{
					"src":"shell/master/surface0018.PNG"
					,"collision":[
						[11,0,121,185,"Brade1"]
					]
					,"kero.balloon.offsetx":30
					,"kero.balloon.offsety":85
				}
				,{
					"src":"shell/master/surface0019.PNG"
					,"collision":[
						[11,0,121,185,"Brade1"]
					]
					,"kero.balloon.offsetx":30
					,"kero.balloon.offsety":85
				}
				,{
					"src":"shell/master/surface0020.PNG"
					,"collision":[
						[8,133,109,350,"Brade0"]
					]
				}
				,{
					"src":"shell/master/surface0021.PNG"
					,"collision":[
						[8,133,109,350,"Brade0"]
					]
				}
				,{
					"src":"shell/master/surface0022.PNG"
					,"collision":[
						[95,67,157,103,"Head"]
						,[104,116,158,143,"Face"]
						,[106,174,155,209,"Bust"]
						,[182,189,209,274,"LeftST"]
					]
					,"interval":[
						{
							"type":"sometimes"
							,"pattern":[
								[63,5,"overlay",100,105]
								,[62,5,"overlay",100,105]
								,[-1,6,"overlay",0,0]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0023.PNG"
					,"collision":[
						[91,62,148,95,"Head"]
						,[97,103,141,143,"Face"]
						,[90,163,138,198,"Bust"]
						,[165,168,195,248,"LeftST"]
					]
					,"interval":[
						{
							"type":"sometimes"
							,"pattern":[
								[65,6,"overlay",90,100]
								,[64,5,"overlay",90,100]
								,[-1,6,"overlay",0,0]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0024.PNG"
					,"collision":[
						[87,64,148,100,"Head"]
						,[95,111,141,143,"Face"]
						,[92,161,144,203,"Bust"]
						,[173,157,207,238,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0025.PNG"
					,"collision":[
						[94,73,156,106,"Head"]
						,[102,110,154,141,"Face"]
						,[109,176,146,212,"Bust"]
						,[184,213,208,281,"LeftST"]
					]
				}
				,{"src":"shell/master/surface0026.PNG"}
				,{
					"src":"shell/master/surface0027.PNG"
					,"collision":[
						[73,68,122,102,"Head"]
						,[82,106,129,145,"Face"]
						,[99,161,149,202,"Bust"]
						,[179,161,213,239,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0028.PNG"
					,"collision":[
						[87,64,148,100,"Head"]
						,[95,111,141,143,"Face"]
						,[92,161,144,203,"Bust"]
						,[173,157,207,238,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0029.PNG"
					,"collision":[
						[86,61,144,101,"Head"]
						,[90,107,146,145,"Face"]
						,[87,178,140,200,"Bust"]
						,[181,164,211,240,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0030.PNG"
					,"collision":[
						[85,79,150,105,"HeadEX"]
						,[94,110,140,138,"FaceEX"]
						,[96,168,139,191,"BustEX"]
						,[154,294,177,341,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0031.PNG"
					,"collision":[
						[85,79,150,105,"HeadEX"]
						,[94,110,140,138,"FaceEX"]
						,[96,168,139,191,"BustEX"]
						,[154,294,177,341,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0032.PNG"
					,"collision":[
						[85,79,150,105,"HeadEX"]
						,[94,110,140,138,"FaceEX"]
						,[96,168,139,191,"BustEX"]
						,[154,294,177,341,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0033.PNG"
					,"collision":[
						[80,62,143,95,"Head"]
						,[91,111,139,142,"Face"]
						,[89,181,140,216,"Bust"]
						,[184,187,211,238,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0034.PNG"
					,"collision":[
						[95,67,157,103,"Head"]
						,[104,116,158,143,"Face"]
						,[106,174,155,209,"Bust"]
						,[182,189,209,274,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0035.PNG"
					,"collision":[
						[80,62,143,95,"Head"]
						,[91,111,139,142,"Face"]
						,[89,181,140,216,"Bust"]
						,[184,187,211,238,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0036.PNG"
					,"collision":[
						[98,65,155,100,"Head"]
						,[100,110,151,141,"Face"]
						,[97,176,151,211,"Bust"]
						,[190,237,216,304,"LeftST"]
					]
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[37,8,"overlay",0,230]
								,[38,8,"overlay",0,230]
								,[39,8,"overlay",0,230]
								,[37,8,"overlay",0,230]
								,[38,8,"overlay",0,230]
								,[39,8,"overlay",0,230]
								,[-1,8,"overlay",0,0]
							]
						}
						,{
							"type":"sometimes"
							,"pattern":[
								[37,6,"overlay",0,230]
								,[38,6,"overlay",0,230]
								,[39,6,"overlay",0,230]
								,[37,6,"overlay",0,230]
								,[38,6,"overlay",0,230]
								,[39,6,"overlay",0,230]
								,[-1,6,"overlay",0,0]
							]
						}
						,{
							"type":"sometimes"
							,"pattern":[
								[37,8,"overlay",0,230]
								,[38,8,"overlay",0,230]
								,[39,8,"overlay",0,230]
								,[37,8,"overlay",0,230]
								,[38,8,"overlay",0,230]
								,[39,8,"overlay",0,230]
								,[-1,8,"overlay",0,0]
							]
						}
						,{
							"type":"sometimes"
							,"pattern":[
								[37,10,"overlay",0,230]
								,[38,10,"overlay",0,230]
								,[39,10,"overlay",0,230]
								,[37,10,"overlay",0,230]
								,[38,10,"overlay",0,230]
								,[39,10,"overlay",0,230]
								,[-1,10,"overlay",0,0]
							]
						}
					]
				}
				,{"src":"shell/master/surface0037.PNG"}
				,{"src":"shell/master/surface0038.PNG"}
				,{"src":"shell/master/surface0039.PNG"}
				,{
					"src":"shell/master/surface0040.PNG"
					,"collision":[
						[80,62,143,95,"Head"]
						,[91,111,139,142,"Face"]
						,[89,181,140,216,"Bust"]
						,[184,187,211,238,"LeftST"]
					]
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[45,8,"overlay",0,0]
								,[46,8,"overlay",0,0]
								,[47,8,"overlay",0,0]
								,[48,8,"overlay",0,0]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0041.PNG"
					,"collision":[
						[80,62,143,95,"Head"]
						,[91,111,139,142,"Face"]
						,[89,181,140,216,"Bust"]
						,[184,187,211,238,"LeftST"]
					]
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[48,8,"overlay",0,0]
								,[47,8,"overlay",0,0]
								,[46,8,"overlay",0,0]
								,[45,8,"overlay",0,0]
								,[43,8,"overlay",0,0]
								,[-1,10,"overlay",0,0]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0042.PNG"
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[81,8,"base"]
								,[82,8,"base"]
								,[83,8,"base"]
								,[84,8,"base"]
								,[85,8,"base"]
								,[86,8,"base"]
								,[87,8,"base"]
								,[88,8,"base"]
								,[89,8,"base"]
								,[43,8,"base"]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0043.PNG"
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[89,8,"base"]
								,[88,8,"base"]
								,[87,8,"base"]
								,[86,8,"base"]
								,[85,8,"base"]
								,[84,8,"base"]
								,[83,8,"base"]
								,[82,8,"base"]
								,[81,8,"base"]
								,[42,8,"base"]
							]
						}
					]
				}
				,{"src":undefined}
				,{"src":"shell/master/surface0045.PNG"}
				,{"src":"shell/master/surface0046.PNG"}
				,{"src":"shell/master/surface0047.PNG"}
				,{"src":"shell/master/surface0048.PNG"}
				,{"src":undefined}
				,{
					"src":"shell/master/surface0050.PNG"
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[52,6,"base"]
								,[53,6,"base"]
								,[54,6,"base"]
								,[55,6,"base"]
								,[56,6,"base"]
								,[57,6,"base"]
								,[58,6,"base"]
								,[51,6,"base"]
							]
						}
					]
				}
				,{
					"src":"shell/master/surface0051.PNG"
					,"interval":[
						{
							"type":"runonce"
							,"pattern":[
								[58,6,"base"]
								,[57,6,"base"]
								,[56,6,"base"]
								,[55,6,"base"]
								,[54,6,"base"]
								,[53,6,"base"]
								,[52,6,"base"]
								,[50,6,"base"]
							]
						}
					]
				}
				,{"src":"shell/master/surface0052.PNG"}
				,{"src":"shell/master/surface0053.PNG"}
				,{"src":"shell/master/surface0054.PNG"}
				,{"src":"shell/master/surface0055.PNG"}
				,{"src":"shell/master/surface0056.PNG"}
				,{"src":"shell/master/surface0057.PNG"}
				,{"src":"shell/master/surface0058.PNG"}
				,{"src":undefined}
				,{"src":"shell/master/surface0060.PNG"}
				,{"src":"shell/master/surface0061.PNG"}
				,{"src":"shell/master/surface0062.PNG"}
				,{"src":"shell/master/surface0063.PNG"}
				,{"src":"shell/master/surface0064.PNG"}
				,{"src":"shell/master/surface0065.PNG"}
				,{"src":undefined}
				,{"src":undefined}
				,{"src":undefined}
				,{"src":undefined}
				,{
					"src":"shell/master/surface0070.PNG"
					,"collision":[
						[84,103,134,136,"Head"]
						,[88,144,132,174,"Face"]
						,[182,191,208,251,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0071.PNG"
					,"collision":[
						[95,67,157,103,"Head"]
						,[104,116,158,143,"Face"]
						,[106,174,155,209,"Bust"]
						,[182,189,209,274,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0072.PNG"
					,"collision":[
						[73,68,122,102,"Head"]
						,[82,106,129,145,"Face"]
						,[99,161,149,202,"Bust"]
						,[179,161,213,239,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0073.PNG"
					,"collision":[
						[75,70,130,106,"Head"]
						,[90,111,132,143,"Face"]
						,[90,188,114,216,"Bust"]
						,[185,169,212,251,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0074.PNG"
					,"collision":[
						[89,81,141,113,"Head"]
						,[99,121,142,152,"Face"]
						,[111,162,146,201,"Bust"]
						,[199,139,228,221,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0075.PNG"
					,"collision":[
						[107,103,161,136,"Head"]
						,[113,140,157,173,"Face"]
						,[131,193,169,213,"Bust"]
						,[201,177,227,241,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0076.PNG"
					,"collision":[
						[98,65,155,100,"Head"]
						,[100,110,151,141,"Face"]
						,[97,176,151,211,"Bust"]
						,[190,237,216,304,"LeftST"]
					]
				}
				,{
					"src":"shell/master/surface0077.PNG"
					,"collision":[
						[89,81,141,113,"Head"]
						,[99,121,142,152,"Face"]
						,[111,162,146,201,"Bust"]
						,[199,139,228,221,"LeftST"]
					]
				}
				,{"src":undefined}
				,{"src":undefined}
				,{"src":undefined}
				,{"src":"shell/master/surface0081.PNG"}
				,{"src":"shell/master/surface0082.PNG"}
				,{"src":"shell/master/surface0083.PNG"}
				,{"src":"shell/master/surface0084.PNG"}
				,{"src":"shell/master/surface0085.PNG"}
				,{"src":"shell/master/surface0086.PNG"}
				,{"src":"shell/master/surface0087.PNG"}
				,{"src":"shell/master/surface0088.PNG"}
				,{"src":"shell/master/surface0089.PNG"}
			]
		}
	}
	,"balloon":{
		"switch_L":{
			"descript":{
				"type":"balloon"
				,"name":"switch_L"
				,"id":"switch_L"
				,"craftman":"Wiz."
				,"craftmanw":"Wiz★"
				,"craftmanurl":"http://www.2chu.net/wiz.stargazer/"
				,"homeurl":"http://ssp.shillest.net/ghost/mobilemaster-balloon/"
				,"origin.x":23
				,"origin.y":10
				,"validrect.left":0
				,"validrect.top":10
				,"validrect.right":0
				,"validrect.bottom":-20
				,"wordwrappoint.x":-30
				,"wordwrappoint.y":0
				,"font.height":12
				,"font.color.r":0
				,"font.color.g":0
				,"font.color.b":30
				,"font.shadowcolor.r":-1
				,"font.shadowcolor.g":-1
				,"font.shadowcolor.b":-1
				,"anchor.font.color.r":38
				,"anchor.font.color.g":53
				,"anchor.font.color.b":153
				,"cursor.blendmethod":"none"
				,"cursor.style":"square"
				,"cursor.brush.color.r":38
				,"cursor.brush.color.g":53
				,"cursor.brush.color.b":153
				,"cursor.pen.color.r":38
				,"cursor.pen.color.g":53
				,"cursor.pen.color.b":153
				,"cursor.font.color.r":255
				,"cursor.font.color.g":255
				,"cursor.font.color.b":255
				,"cursor":"NULL"
				,"arrow0.x":310
				,"arrow0.y":6
				,"arrow1.x":310
				,"arrow1.y":-16
				,"onlinemarker.x":0
				,"onlinemarker.y":0
				,"sstpmarker.x":16
				,"sstpmarker.y":-17
				,"sstpmessage.x":28
				,"sstpmessage.y":-18
				,"sstpmessage.font.height":10
				,"sstpmessage.font.color.r":0
				,"sstpmessage.font.color.g":0
				,"sstpmessage.font.color.b":100
				,"number.font.height":10
				,"number.font.color.r":0
				,"number.font.color.g":0
				,"number.font.color.b":100
				,"number.xr":-28
				,"number.y":-18
				,"communicatebox.x":18
				,"communicatebox.y":11
				,"communicatebox.width":250
				,"communicatebox.height":20
			}
			,"blimp":[
				{
					"s":{
						"src":"switch_L/balloons0.png"
						,"windowposition.x":0
						,"windowposition.y":0
					}
					,"k":{
						"src":"switch_L/balloonk0.png"
						,"windowposition.x":0
						,"windowposition.y":0
					}
					,"c":{
						"src":"switch_L/balloonc0.png"
					}
				}
				,{
					"s":{
						"src":"switch_L/balloons1.png"
						,"windowposition.x":0
						,"windowposition.y":0
					}
					,"k":{
						"src":"switch_L/balloonk1.png"
						,"windowposition.x":0
						,"windowposition.y":0
					}
					,"c":{
						"src":"switch_L/balloonc1.png"
					}
				}
				,{
					"s":{
						"src":"switch_L/balloons2.png"
						,"windowposition.x":0
						,"windowposition.y":0
						,"onlinemarker.x":0
						,"onlinemarker.y":100
					}
					,"c":{
						"src":"switch_L/balloonc2.png"
					}
				}
				,{
					"s":{
						"src":"switch_L/balloons3.png"
						,"windowposition.x":0
						,"windowposition.y":0
						,"onlinemarker.x":0
						,"onlinemarker.y":100
					}
					,"c":{
						"src":"switch_L/balloonc3.png"
					}
				}
				,{
					"c":{
						"src":"switch_L/balloonc4.png"
					}
				}
			]
			,"online":[
				{"src":"switch_L/online0.png"}
			]
			,"arrow":[
				{"src":"switch_L/arrow0.png"}
				,{"src":"switch_L/arrow1.png"}
			]
			,"cancel":[
				{"src":"switch_L/cancel_down.png"}
				,{"src":"switch_L/cancel_up.png"}
			]
			,"mode":[
				{"src":"switch_L/mode_down.png"}
				,{"src":"switch_L/mode_up.png"}
			]
			,"ok":[
				{"src":"switch_L/ok_down.png"}
				,{"src":"switch_L/ok_up.png"}
			]
			,"sstp":{"src":"switch_L/sstp.png"}
		}
	}
}