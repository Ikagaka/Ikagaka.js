						...
						"surface": (function(){
							...
							return (function(){
								...
								return function(num,callback){
									...
									if(typeof surface[curSurface].element !== "undefined"){//もしエレメント合成が定義されていたら
										try{//エレメント合成
											for(var i = 0; i < surface[curSurface].element.length; i++){
												var ctx = canvas.getContext('2d');
												ctx.drawImage(surface[curSurface].element[i].canvas,surface[curSurface].element[i][3],surface[curSurface].element[i][4]);
											}
										}catch(e){//エレメント合成に失敗
