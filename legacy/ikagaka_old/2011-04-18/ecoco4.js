function ecoco(homeurl, callback){
	var jsnNar = {
		...
		file = {};
	function loadSurfaces(surface, structure){
		surface = surface || {};
							...
							success: (function(jsnShell){
								...
								return function(data){
									...
									$.each(data.match(/surface[^{}]+\{[^{}]+\}/gim), function(i, str){
										...
										if(isFinite(num)){
											jsnShell.surface[num] = loadSurfaces(jsnShell.surface[num],aryStructure[2]);
										}else if(/(.append)?([!\d\,\-]+)/.test(aryStructure[1])){
											console.log(aryStructure[1])
											...
											while(i--){
												if(aryBool[i]){
													...
													jsnShell.surface[i] = loadSurfaces(jsnShell.surface[i],aryStructure[2]);
												}
											}
										}
									});
